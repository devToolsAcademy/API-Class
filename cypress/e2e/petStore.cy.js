describe("Pet Store", () => {
    ...
    context("API Testing", () => {
        ...
        it("GET - Find pets by status", () => {
            cy.request("v2/pet/findByStatus?status=available").then((resp) =>
        ...
        it("GET - Find pet by ID", () => {
            cy.request(`v2/pet/${id}`).then((response) =>
                expect(response.status).to.eq(200)
            );
        });

        it("DELETE - Delete pet", () => {
            cy.request("DELETE", `v2/pet/${id}`).then((response) =>
                expect(response.status).to.eq(200)
            );
        });

        it("GET - Verify pet does not exist", () => {
            cy.request({ url: `v2/pet/${id}`, failOnStatusCode: false }).then(
                (resp) => expect(resp.status).to.eq(404)
            );
        });
    });
    ...
    context("API request + UI Assertions", () => {
        ...
        it("Verify pet exist", () => {
            ...
            cy.get(
                ".live-responses-table .response .response-col_status"
